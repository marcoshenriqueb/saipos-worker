...
async function main(): Promise<void> {
  ...
  console.log(
    `🚚 Backfill orders_raw | start=${startDateStr} end=${endDateStr} (exclusive) windowDays=${windowDays} sleepMs=${sleepMs}` +
  ...
  for (let cur = new Date(start); cur < endExclusive; ) {
    ...
    console.log(
      `📦 window ${toDateOnlyUtc(cur)}..${toDateOnlyUtc(winEnd)} sales=${sales.length} items=${items.length} upserted=${upserted}`
    );
